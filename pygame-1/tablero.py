...
class Tablero:
        ...
        for x in range(0,CANTIDAD_TARJETAS_H*ANCHO_TARJETA,ANCHO_TARJETA):
            for y in range(0,CANTIDAD_TARJETAS_V*ALTO_TARJETA,ALTO_TARJETA):
                if(i > CANTIDAD_TARJETAS_UNICAS):
                    tarjeta_test = Tarjeta("\\0{0}.png".format(i-CANTIDAD_TARJETAS_UNICAS),r"\\00.png",x,y)
                else:
                    tarjeta_test = Tarjeta("\\0{0}.png".format(i),r"\\00.png",x,y)
    ...
    def colicion(self,pos_xy):
        ...
        if(Tarjeta.cantidad_tarjetas_visibles_no_descubiertas(Tarjeta,lista_tarjetas) < 2):
    ...
    def update(self):
        ...
        if(self.tiempo_ultimo_destape > 0):
            if(Tarjeta.match(Tarjeta, self.l_tarjetas)):
                self.tiempo_ultimo_destape = 0
